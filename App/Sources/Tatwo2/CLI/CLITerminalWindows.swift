// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/CLITerminalWindows.swift；改動 2 行（原因：加入來源標記；移除舊 Core import，改由 Facade 同名假資料型別供應）
import AppKit
import SwiftUI
...
struct WindowTrafficLightVisibilitySync: NSViewRepresentable {
    ...
    /// W112：獨立 Browser 自己管紅綠燈（頂列隱形時一起藏），這裡就不要每次重算都把它們叫回來。
    var managedByBrowser = false

    func makeCoordinator() -> Coordinator { Coordinator() }

    func makeNSView(context: Context) -> NSView {
        let view = NSView(frame: .zero)
        if !managedByBrowser { context.coordinator.attach(to: view, sidebarPinned: sidebarPinned) }
        return view
    }

    func updateNSView(_ nsView: NSView, context: Context) {
        if managedByBrowser { return }
