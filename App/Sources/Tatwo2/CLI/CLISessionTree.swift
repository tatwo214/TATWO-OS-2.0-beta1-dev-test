// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/CLISessionTree.swift；改動 10 行（原因：保留既有改動，cli-ui 最小掛接；新增 UI 邏輯在 New、資料在 Facade）
import SwiftUI
...
struct CLISessionTree: View {
    ...
    var openSessionsContent: AnyView? = nil
    ...
    private var terminalSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            if openSessionsContent == nil && openSessions.isEmpty && projects.isEmpty {
                emptyState("尚無 CLI 專案。點 + 新增資料夾或匯入 chat 專案。")
            } else {
                if let openSessionsContent {
                    openSessionsContent
