...
final class BrowserWorkSpaceStore: ObservableObject {
    struct Space: Identifiable, Equatable {
        ...
        var registryID: UUID? = nil, color: String? = nil
    }
    ...
    @Published var hoverRailShown = false   // W114：側欄沒固定、被滑鼠喚出來的時候（紅綠燈與收納鈕要跟著出現）
    func toggleSidebar() {
    ...
    /// W120（使用者 2026-09-21：「按- 應該是回到browser的預設首頁」）：「-」關掉的若是正在看的分頁，留在首頁，不跳到清單第一個。
    func closeBookmark(_ id: UUID) {
        if let tab = tab(forBookmark: id) { stayOnStartPage = stayOnStartPage || tab.id == selectedID; close(tab.id) }
    }
    func folderHasOpenTabs(_ id: UUID) -> Bool { tabs.contains { $0.folderID == id } }
    func closeFolderTabs(_ id: UUID) {
        let closing = tabs.filter { $0.folderID == id }
        stayOnStartPage = stayOnStartPage || closing.contains { $0.id == selectedID }
        for tab in closing { close(tab.id) }
    }
    ...
    func saveDraggedTabs(_ payloads: [String], into folderID: UUID) -> Bool {
        ...
        for payload in payloads where payload.hasPrefix("tatwo-browser-tab:") {
            ...
            // W115（自測 .013）：拖進資料夾＝搬進去——分頁歸這個書籤、住在書籤列，不留一份在下面。
            guard let uuid = tabIDs[id], let tab = registry.tabs.first(where: { $0.id == uuid }), let url = tab.url,
                  let bookmark = registry.addBookmark(folderID: folderID, url: url, title: tab.title) else { continue }
            registry.bind(tab: uuid, toBookmark: bookmark.id, folderID: folderID); saved = true
        }
        return saved
    }
    ...
    struct Tab: Identifiable, Equatable {
        ...
        var faviconPNG: Data?
        var registryID: UUID?
        var bookmarkID: UUID?, favoriteID: UUID?
        var folderID: UUID?
        var loading = false
    }
    ...
    // 選到任何真實分頁就離開「留在首頁」；W144：也通知擴充管理頁收起（它的行為要像一個分頁，切走就不見）
    @Published private(set) var selectedID = 0 { didSet { if selectedID >= 0 { stayOnStartPage = false }; NotificationCenter.default.post(name: .init("tatwo.browser.workspace.selection"), object: nil) } }
    private var stayOnStartPage = false
...
struct BrowserWorkSpaceDesignView: View {
    ...
    var body: some View {
        Group {
            if onClose != nil {
                ...
                // W112：頂列平時隱形、網頁貼頂；滑鼠到視窗最上緣才浮出（見 BrowserChromeReveal）。
            ...
            .task(id: store.searchFocusRequest) {
                ...
                else { chromeReveal.revealForFocus(); await Task.yield(); addressExpansionRequested = true }   // ⌘L：頂列平時不在，先叫出來再聚焦
            }
    ...
    private func restoreAddress() {
        query = BrowserChromeStyleEmbedState.shared.url != nil ? "擴充功能" : (store.showsStartPage ? "" : store.selectedTab.url)
    }
        ...
        VStack(spacing: BrowserOmniboxMetrics.zero) {
            HStack(spacing: BrowserOmniboxMetrics.controlGap) {
                ...
                EmbeddedBrowserToolbar(addressText: $query, addressFieldFocused: $addressFocused,
                    ...
                    newTabIntent: $newTabIntent, showsAddress: !store.showsStartPage || BrowserChromeStyleEmbedState.shared.url != nil, labelOverride: BrowserChromeStyleEmbedState.shared.url != nil ? "擴充功能" : nil, compactChrome: onClose != nil, isEditing: $addressEditing)
    ...
    func performBrowserAction(_ action: BrowserAction) {
        ...
        case .newTab:   // ⌘T：看著網頁時不換頁，叫出空白搜尋面板；使用者 09-20：「再點一次是取消搜尋」
            ...
            if store.canAddTab { if store.showsStartPage { store.addTab() } else { newTabIntent = true }; store.searchFocusRequest += 1 }
    ...
    private func submitSearch() {
        ...
        if newTabIntent { newTabIntent = false; store.addTab(url: url); addressFocused = false; focusedField = nil; return }   // ⌘T 的送出：開成新分頁並切過去
