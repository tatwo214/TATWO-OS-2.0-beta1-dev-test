// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/EmbeddedBrowserWebView.swift；改動 3 行（原因：加入照搬來源標記；移除薄殼不可依賴的 TatwoCEFBridge、TatwoUltraworkCore import）
import AppKit
import SwiftUI
import WebKit
import Darwin
...
struct EmbeddedBrowserWebView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        do {
            if let initialAgentNavigation {
                _ = try BrowserAgentBridge.shared.enqueueBrowserNavigation(
                    initialAgentNavigation, url: initialURL,
                    profileID: profile.dataStoreIdentifier, enqueue)
            } else {
                enqueue()
            }
            context.coordinator.publishState(for: webView)
        } catch {
            context.coordinator.record(.runtimeMessage("瀏覽器操作已撤回，請重新觀察後再試。"), for: webView)
        }
        return webView
    }
    ...
    func updateNSView(_ webView: WKWebView, context: Context) {
        ...
        do {
            if let navigation = command.agentNavigation {
                ...
                _ = try BrowserAgentBridge.shared.enqueueBrowserNavigation(
                    navigation, url: url, profileID: profile.dataStoreIdentifier, enqueue)
            } else {
                enqueue()
            }
        } catch {
    ...
    {
        let leaseID = UUID()
        private(set) var agentNavigationID = UUID()
        let profile: EmbeddedBrowserRuntimeProfile
        ...
        func navigationActionPolicy(
            url: URL?, targetIsMainFrame: Bool?, shouldDownload: Bool,
            ...
            guard targetIsMainFrame != nil else {
        ...
        func navigationResponsePolicy(
            response: URLResponse, isMainFrame: Bool, canShowMIMEType: Bool,
            in webView: WKWebView
