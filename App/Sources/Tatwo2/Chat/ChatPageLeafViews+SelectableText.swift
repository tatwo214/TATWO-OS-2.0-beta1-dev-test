// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/ChatPageLeafViews+SelectableText.swift；改動 16 行（原因：移除舊 Core import，改接同名 Facade 假資料）
import SwiftUI
import AppKit
import Foundation
import QuickLook
...
private final class ChatFlowSelectionCoordinator {
    ...
    func hasSelection(in window: NSWindow?) -> Bool {
        guard let window, sessionWindow === window else { return false }
        return selection.contains { $0.view.window === window && $0.range.length > 0 }
    }
    ...
    @discardableResult
    func copySelection(in window: NSWindow?, to pasteboard: NSPasteboard = .general) -> Bool {
        guard hasSelection(in: window) else { return false }
        let ordered = selection.filter { $0.view.window === window }.sorted { lhs, rhs in
        ...
        let parts: [String] = ordered.compactMap { view, range in
            ...
            let bounded = NSRange(location: range.location, length: min(range.length, storage.length - range.location))
            guard bounded.length > 0 else { return nil }
            return (storage.string as NSString).substring(with: bounded)
        }
    ...
    private func handleKeyDown(_ event: NSEvent) -> NSEvent? {
        ...
        if key == "c" {
            ...
            if copySelection(in: event.window) {
                return nil
            }
            return event
        }
...
/// AppKit action/selection regression; uses hidden windows and a private
/// pasteboard, never the user's clipboard or conversation store.
enum ChatSelectionAcceptance {
    @MainActor static func run() -> Bool {
        let coordinator = ChatFlowSelectionCoordinator.shared
        ...
        defer {
            coordinator.selectAllVisible(in: other)
        ...
        func check(_ name: String, _ success: Bool) {
            ...
            print("CHATSELECTIONTEST \(success ? "PASS" : "FAIL") \(name)")
        }
        func flow(_ text: String, y: CGFloat) -> ChatFlowTextView {
            let view = ChatFlowTextView(frame: NSRect(x: 10, y: y, width: 450, height: 60))
        ...
        // Insert out of order to prove copy order comes from the transcript layout.
        let lower = flow("第二則\n換行 🧑‍💻", y: 200)
        let upper = flow("第一則 中文", y: 350)
        let hidden = flow("不可洩漏的隱藏文字", y: 420)
        hidden.isHidden = true
        let composer = NSTextView(frame: NSRect(x: 10, y: 10, width: 450, height: 80))
        composer.string = "尚未送出的草稿"
        window.contentView?.addSubview(composer)
        _ = window.makeFirstResponder(composer)
        check("fixture starts with composer focus", window.firstResponder === composer)
        if let click = NSEvent.mouseEvent(with: .leftMouseDown, location: NSPoint(x: 25, y: 370),
                                         modifierFlags: [], timestamp: 0, windowNumber: window.windowNumber,
                                         context: nil, eventNumber: 1, clickCount: 1, pressure: 1) {
            upper.mouseDown(with: click)
        }
        check("transcript click moves focus out of composer", window.firstResponder === upper)
        upper.selectAll(nil)
        check("select-all action creates cross-message selection", coordinator.hasSelection(in: window))
        check("temporary highlight does not require native selection", upper.selectedRange().length == 0)
        let copyItem = NSMenuItem(title: "拷貝", action: #selector(NSText.copy(_:)), keyEquivalent: "c")
        check("native Copy menu stays enabled for highlights", upper.validateUserInterfaceItem(copyItem))
        copyItem.target = upper
        let menu = NSMenu(title: "fixture")
        menu.addItem(copyItem)
        menu.update()
        check("AppKit menu auto-validation enables highlighted copy", copyItem.isEnabled)
        check("cross-message copy writes private clipboard", coordinator.copySelection(in: window, to: pasteboard))
        check("copy keeps order, unicode and line breaks", pasteboard.string(forType: .string) == "第一則 中文\n第二則\n換行 🧑‍💻")
        check("copy excludes composer draft", pasteboard.string(forType: .string)?.contains(composer.string) == false)
        check("copy excludes hidden transcript", pasteboard.string(forType: .string)?.contains(hidden.string) == false)
        check("another window cannot copy this selection", !coordinator.copySelection(in: other, to: pasteboard))
        check("no-window copy is rejected", !coordinator.copySelection(in: nil, to: pasteboard))
        let before = pasteboard.string(forType: .string)
        check("rejected copy preserves clipboard", before == "第一則 中文\n第二則\n換行 🧑‍💻")
        // Shrinking/replacing a selected row cannot make all selected text uncopyable.
        lower.string = "短"
        check("changed text storage remains safe to copy", coordinator.copySelection(in: window, to: pasteboard))
        check("stale range is clamped to current storage", pasteboard.string(forType: .string) == "第一則 中文\n短")
        upper.setSelectedRange(NSRange(location: 0, length: 3))
        check("native word selection clears synthetic highlight", !coordinator.hasSelection(in: window))
        check("native word Copy menu still works", upper.validateUserInterfaceItem(copyItem))
        check("native selection does not write stale aggregated text", !coordinator.copySelection(in: window, to: pasteboard))
        print("CHATSELECTIONTEST RESULT passed=\(passed) failed=\(failed) skipped=0")
        return failed == 0
    }
}
...
final class ChatCodeBlockAccessoryView: NSView {
    ...
    /// 2026-09-11 使用者回饋：白板對齊 Claude 官方樣式——右上只放兩顆淡色圖示鍵，不用系統方框按鈕。
    init(code: String, language: String) {
        self.code = code
        self.language = language
        copyButton = Self.iconButton("doc.on.doc", label: "複製程式碼", tip: "複製")
        horizontalViewerButton = Self.iconButton(
            "arrow.left.and.right", label: "水平檢視程式碼", tip: "橫向檢視")
    ...
    private static func iconButton(_ symbol: String, label: String, tip: String) -> NSButton {
    ...
    override func layout() {
        ...
        horizontalViewerButton.frame = NSRect(x: 26, y: 0, width: 22, height: 22)
    }

    @objc private func copyCode(_ sender: Any?) {
        copyAction?(code)
        let original = copyButton.image
        copyButton.image = NSImage(systemSymbolName: "checkmark", accessibilityDescription: "已複製")?
...
final class ChatFlowTextView: NSTextView, NSTextViewDelegate {
    ...
    // 程式碼區塊：圓角淡底＋髮絲邊（Claude 官方樣式）。NSTextBlock 只負責留 12pt 內距。
    ...
    private func drawCodeBlockBackgrounds() {
        ...
        storage.enumerateAttribute(
            ...
            let rect = NSRect(x: origin.x, y: origin.y + used.minY - 12,
                              width: textContainer.size.width, height: used.height + 24)
            // 2026-09-11 使用者：灰框保留、試液態玻璃。Dashboard「玻璃 1」：R28、tint #ffffff α0.18、
            // 陰影 0.04 (0,4) 14。blur 2.2／distortion 1.3／chromatic 1.5 在純紙色底上取樣不到內容、
            // NSTextView 內也跑不了 shader → 降級不畫。灰底 0.045 與光邊漸層是工程合成值。
            // 使用者 2026-09-11：Dashboard 的 R28 太圓 → 改 12（與 chip 圓角同級）；材質參數仍照 Dashboard。
            ...
            NSGradient(colors: [NSColor.white.withAlphaComponent(0.18),
                                NSColor.white.withAlphaComponent(0.18 * 0.35)])?
                .draw(in: rect, angle: 90)
            NSGraphicsContext.restoreGraphicsState()
            if let context = NSGraphicsContext.current?.cgContext {
                context.saveGState()
                context.addPath(path.cgPath.copy(strokingWithWidth: 1, lineCap: .round,
                                                 lineJoin: .round, miterLimit: 1))
                context.clip()
                NSGradient(colors: [NSColor.white.withAlphaComponent(0.85),
    ...
    // Temporary cross-message highlights are not NSTextView's selectedRange.
    // Route both menu actions and shortcuts through the same selection.
    override func copy(_ sender: Any?) {
        if !ChatFlowSelectionCoordinator.shared.copySelection(in: window) {
            super.copy(sender)
        }
    }

    override func selectAll(_ sender: Any?) {
        guard let window else { super.selectAll(sender); return }
        ChatFlowSelectionCoordinator.shared.selectAllVisible(in: window)
    }

    override func validateUserInterfaceItem(_ item: NSValidatedUserInterfaceItem) -> Bool {
        if item.action == #selector(copy(_:)),
           ChatFlowSelectionCoordinator.shared.hasSelection(in: window) { return true }
    ...
    override func menu(for event: NSEvent) -> NSMenu? {
        ...
        if ChatFlowSelectionCoordinator.shared.hasSelection(in: window) {
            let item = NSMenuItem(
                title: "複製選取",
    ...
    @objc private func copySelectionFromCoordinator(_ sender: Any?) {
        ChatFlowSelectionCoordinator.shared.copySelection(in: window)
    }
    ...
    private func positionCodeBlockAccessories() {
        ...
        for entry in codeBlockAccessoryViews {
            ...
            entry.view.frame.origin = NSPoint(
                x: max(
                    origin.x,
                    origin.x + textContainer.size.width - entry.view.frame.width - 8),
                y: origin.y + rect.minY - 8)
        }
    }
...
final class ChatStreamingPlainTextStorageController {
    ...
    @MainActor
    init(
        layoutMeasurer: ChatStreamingTextLayoutMeasuring? = nil
    ) {
        self.layoutMeasurer = layoutMeasurer ?? ChatDefaultStreamingTextLayoutMeasurer()
    }
