// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/ChatPageLeafViews.swift；改動 4 行（原因：移除舊 Core import，改接同名 Facade 假資料）
import SwiftUI
import AppKit
import Foundation
import QuickLook
        ...
        if !attachments.isEmpty {
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 128, maximum: 220), spacing: 8)], alignment: .leading, spacing: 8) {
                ForEach(visibleAttachments) { attachment in
                    ChatAttachmentTile(attachment: attachment, gallery: imageAttachments)
                }
...
private struct ChatAttachmentTile: View {
    ...
    var body: some View {
        if attachment.isImage {
            ChatLoadedImageAttachment(attachment: attachment, gallery: gallery, size: 112)
                .frame(maxWidth: .infinity, alignment: .leading)
        } else {
            Button {
...
struct ChatBubble: View {
    ...
    private var userMessageBubble: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            if let status = message.status,
               status.hasPrefix("steering|") || status.hasPrefix("steer_failed|") || status.hasPrefix("steer_unknown|") {
                Text(String(status.split(separator: "|", maxSplits: 1).last ?? ""))
                    .font(ChatTypography.transcriptMeta)
                    .foregroundStyle(status.hasPrefix("steering|") ? Color.secondary : Color.orange)
            }
    ...
    private var rowBackground: some ShapeStyle {
        ...
        // 使用者訊息用主題錨色（fable5＝赤陶、極光＝極光色），不再用系統藍 .accentColor。
        let isUser = message.role == .user && message.eventKind != .toolUse
        let tint = isUser ? LiquidGlassTokens.brandAccent : rowTint
        return LinearGradient(
            colors: [
                tint.opacity(firstOpacity),
                isUser ? tint.opacity(firstOpacity) : Color.white.opacity(secondOpacity)
            ],
    ...
    private var rowStroke: Color {
        ...
        if message.role == .user {
            return LiquidGlassTokens.brandAccent.opacity(
                TatwoChatTranscriptVisualMetrics.userBubbleStrokeOpacity)
        }
        return rowTint.opacity(0.058)
    }
}
