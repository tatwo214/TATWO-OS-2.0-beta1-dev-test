...
extension ChatPage {
    ...
    /// 資訊卡（Codex 摘要切換符號）／ 聊天旁瀏覽器 ／ 工具箱。同色同尺寸、無框。左列固定鈕 09-20 起搬到左上（見 sidebarPinButton）。
    ...
    /// 使用者 2026-09-20：「chat分頁的work space固定鈕位置改跟bowerser的固定鈕一樣 包含切換位置變化」。
    /// 與 Browser 同一顆字形、同尺寸：未固定時在紅綠燈右邊，固定後在側欄右緣外側（內容區左上）。
    ...
    func rightPanelControlStrip(showsThreadControls: Bool) -> some View {
        HStack(spacing: 8) {
            ...
            // W104「變更」：只有這條聊天的工作資料夾真的有未提交的變更時才出現（附檔案數），平常不佔位置、不提醒。
            if let summary = model.workspaceChangeSummary {
                Button { applyRightPanelInteraction(.toggleDiff) } label: {
                    HStack(spacing: 3) {
                        Image(systemName: "plus.forwardslash.minus").font(.system(size: 12.5, weight: .medium))
                        Text("\(summary.files)").font(.system(size: 10.5, weight: .semibold).monospacedDigit())
                    }
                    .foregroundStyle(rightPanelContent == .diff && isRightPanelOpen ? LiquidGlassTokens.brandAccent : Color.secondary)
                    .frame(height: 26).padding(.horizontal, 4)
                    .contentShape(RoundedRectangle(cornerRadius: 7, style: .continuous))
                }
                .buttonStyle(.plain)
                .help("這條聊天改了 \(summary.files) 個檔案（+\(summary.added) −\(summary.removed)），點一下看逐行差異")
                .accessibilityLabel("變更：\(summary.files) 個檔案")
                .accessibilityIdentifier("chat.changes.toggle")
            }

            // 工具箱（使用者 09-19）：跟瀏覽器的 ⌃ 一樣，滑鼠移入就往下展開一列浮空圓鈕；點一下固定、再點收合。
            // 「瀏覽器」已有自己的地球鈕，不再放這裡；「變更收據 (diff)」實機是壞鍵，先從工具箱拿掉
            //（W104 已重做成頂列的「變更」鈕＋新面板，見 ChatWorkspaceChangesView。）
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            Group {
                ...
                case .loops:
                    ...
                    .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading)
                case .diff:
                    ChatWorkspaceChangesView(load: { await model.loadWorkspaceChanges() },
                                             onClose: { applyRightPanelInteraction(.toggleDiff) })
