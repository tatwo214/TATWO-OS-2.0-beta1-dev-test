...
struct ChatComposerTextView: NSViewRepresentable {
    ...
    final class ComposerNSTextView: NSTextView {
        ...
        // 純文字輸入框遇到「剪貼簿裡只有圖片」（截圖、在照片或預覽程式按拷貝）時，系統會把「貼上」停用，
        // ⌘V 完全沒反應、走不到上面的附件管線（09-25 實機：ChatGPT 輸入框貼不進照片的根因）。
        // 有附件管線時，剪貼簿是圖片、檔案或檔案承諾也讓「貼上」可以按。
        override func validateUserInterfaceItem(_ item: NSValidatedUserInterfaceItem) -> Bool {
            if item.action == #selector(paste(_:)), onPasteImage != nil, Self.carriesAttachment(NSPasteboard.general) {
                return true
            }
            return super.validateUserInterfaceItem(item)
        }

        // 「編輯 › 貼上」與 ⌘V 走的是選單驗證（validateMenuItem），NSTextView 不會轉給上面那個方法
        // （09-25 v2.0.19.021 實機：輸入框是焦點、剪貼簿是圖片，選單上的「貼上」仍是停用）。
        override func validateMenuItem(_ menuItem: NSMenuItem) -> Bool {
            if menuItem.action == #selector(paste(_:)), onPasteImage != nil, Self.carriesAttachment(NSPasteboard.general) {
                return true
            }
        ...
        // 「照片」App、瀏覽器拖出來的圖是「檔案承諾」，純文字輸入框預設不收（游標變禁止、拖不進來）；
        // 有附件管線（onPasteImage）時也收這種拖曳（09-25 使用者「照片抓不進去」）。
        override var acceptableDragTypes: [NSPasteboard.PasteboardType] {
            guard acceptsPhotoDrags, onPasteImage != nil else { return super.acceptableDragTypes }
            let extra = NSFilePromiseReceiver.readableDraggedTypes.map { NSPasteboard.PasteboardType($0) } + [.fileURL, .png, .tiff]
            return super.acceptableDragTypes + extra.filter { !super.acceptableDragTypes.contains($0) }
        }

        private func carriesAttachment(_ info: NSDraggingInfo) -> Bool {
            guard acceptsPhotoDrags, onPasteImage != nil else { return false }
