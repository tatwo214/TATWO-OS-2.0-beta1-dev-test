// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/ChatPageLeafViews+WorkTimeline.swift；改動 4 行（原因：移除舊 Core import，改接同名 Facade 假資料）
import SwiftUI
import AppKit
import Foundation
import QuickLook
...
enum ChatTranscriptDisplayBuilder {
    ...
    static func build(_ messages: [ChatMessage]) -> [ChatTranscriptDisplayItem] {
        ...
        // 2026-09-11 使用者回饋：做完的工作摘要放在這一輪 AI 訊息的最後面，不擋在回覆前面。
        // 進行中的仍留在原位（回覆還沒出來）。
        var lastIndexByTurn: [String: Int] = [:]
        for (index, message) in messages.enumerated() {
            lastIndexByTurn[message.turnID ?? message.id] = index
        }
        var deferredByTurn: [String: [ChatInlineWorkTimeline]] = [:]
        func flushDeferred(_ turnID: String) {
            guard let pending = deferredByTurn.removeValue(forKey: turnID) else { return }
            result.append(contentsOf: pending.map(ChatTranscriptDisplayItem.workTimeline))
        }
        for (index, message) in messages.enumerated() {
            defer {
                let turnKey = message.turnID ?? message.id
                if lastIndexByTurn[turnKey] == index { flushDeferred(turnKey) }
            }
            ...
            if timeline.presentation.isActive {
                result.append(.workTimeline(timeline))
            } else {
                deferredByTurn[workScope.turnID, default: []].append(timeline)
            }
        }
        for turnKey in deferredByTurn.keys.sorted() { flushDeferred(turnKey) }
        return result
    }
...
struct ChatInlineWorkTimelineView: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 4) {
            Button {
                ...
                if !summary.isActive {
                    finishedToggle(summary)
                } else {
                HStack(spacing: 5) {
                    if summary.isActive && timeline.presentation.state != .reconnecting {
                        ChatTypingDots()
                    } else {
                    ...
                    Text(summary.label)
                        ...
                        .lineLimit(1)
                    if !summary.compactText.isEmpty && (isExpanded || timeline.presentation.state == .failed || timeline.presentation.state == .reconnecting) {
    ...
    /// 做完的一輪只留一顆向下箭頭圓鈕；展開後旁邊才寫「已完成 · N 個步驟」。
    private func finishedToggle(_ summary: ChatInlineWorkTimelineSummary) -> some View {
        let failed = summary.state == .failed
        return HStack(spacing: 6) {
            Image(systemName: isExpanded ? "chevron.up" : "chevron.down")
                .font(.system(size: 7, weight: .bold))
                .foregroundStyle(failed ? AnyShapeStyle(Color.red.opacity(0.8)) : AnyShapeStyle(.secondary))
                .frame(width: 16, height: 16)
                // 2026-09-11 使用者：圓鈕給液態玻璃。Dashboard「玻璃 1」：白 tint α0.18、陰影 0.04 (0,4) 14；
                // 光邊漸層是工程合成值（純紙底上模糊／折射／色散看不出，降級不畫）。
                .background {
                    Circle().fill(.ultraThinMaterial)
                        .overlay(Circle().fill(Color.white.opacity(0.18)))
                }
                .overlay(Circle().strokeBorder(
                    LinearGradient(colors: [Color.white.opacity(0.85), Color.primary.opacity(0.10)],
        ...
        .contentShape(Rectangle())
        // 2026-09-11 使用者：圓鈕對齊 AI 文字左緣、不跟 AI 頭像同列（頭像 24＋間距 11，實測 35pt）。
        .padding(.leading, Self.assistantTextInset)
    }

    private static let assistantTextInset: CGFloat = 35
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ForEach(Array(files.prefix(8))) { file in
                    ...
                    .buttonStyle(.plain)
                    .help("打開 diff 面板查看這個檔案")
                    .accessibilityElement(children: .combine)
                    .accessibilityLabel(
                        "\(file.path)，新增 \(file.additions) 行，刪除 \(file.deletions) 行")
                    .accessibilityIdentifier("chat-changed-file-\(file.path)")
                }
