// 照搬自 Packages/TatwoUltraworkCore/Sources/TatwoUltraworkCore/SlashCommandParser.swift；改動 2 行（原因：改接 Tatwo2 同名 Facade 假資料）
import Foundation
...
public enum TatwoSlashCommandParser {
  public static func prArgument(in raw: String) -> String? {
    let input = raw.trimmingCharacters(in: .whitespacesAndNewlines)
    guard let token = input.split(maxSplits: 1, whereSeparator: \.isWhitespace).first,
          token == "/pr" else { return nil }
    return String(input.dropFirst(3)).trimmingCharacters(in: .whitespacesAndNewlines)
  }

  /// Feedback is a leading standalone command, never text inside a note/code fence.
  public static func feedbackArgument(in raw: String) -> String? {
    let input = raw.trimmingCharacters(in: .whitespacesAndNewlines)
    let command = "/feedback"
    guard input == command || input.hasPrefix(command + " ") || input.hasPrefix(command + "\n")
        || input.hasPrefix(command + "\t") else { return nil }
    return String(input.dropFirst(command.count)).trimmingCharacters(in: .whitespacesAndNewlines)
  }
