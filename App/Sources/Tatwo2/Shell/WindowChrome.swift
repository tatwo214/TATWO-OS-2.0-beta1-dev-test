// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/WindowChrome.swift；改動 1 行（原因：run A 照搬，僅移除舊水電 import／呼叫並接同名 Facade）
import AppKit
import SwiftUI
...
final class TatwoWindowDragNSView: NSView {
    /// W112：獨立 Browser 的頂列隱形時，網頁貼到視窗頂；這條拖曳區要讓開，點擊才到得了網頁。
    @MainActor static var suppressed = false
    override func hitTest(_ point: NSPoint) -> NSView? { Self.suppressed ? nil : super.hitTest(point) }
    override var mouseDownCanMoveWindow: Bool { !Self.suppressed }
