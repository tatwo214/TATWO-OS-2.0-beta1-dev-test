...
extension Notification.Name {
    ...
    /// 打開設定到指定的區（object＝TatwoSettingsPage.Section 的 rawValue）；例：ChatGPT Space 要登入時開到 Plugin › TAP。
    static let tatwoOpenSettingsSection = Notification.Name("tatwoOpenSettingsSection")
...
enum TatwoUltraworkMacApp {
    ...
    static func main() {
        ...
        // 瀏覽器核心只在啟動時讀「受保護的音樂與影片」開關；先記下這次啟動的值與時間，設定頁才知道要不要提示重開。
        _ = (BrowserProtectedMedia.enabledAtLaunch, BrowserProtectedMedia.launchedAt)
    ...
    var body: some View {
        HStack(spacing: WindowChromeMetrics.controlSpacing) {
            Color.clear
                ...
                // W112：Chat／CLI 的側欄固定鈕坐在側欄右緣外側；拖曳區多讓出這一格，不然真實點擊會被拖曳 NSView 吃掉。
                .frame(width: showRightPanelToggle ? WindowChromeMetrics.trafficLightSafeWidth : WorkspaceSidebarMetrics.width + ChatPage.sidebarPinButtonReserve)
                .allowsHitTesting(false)
