...
enum TatwoSettingsPageMetrics {
    /// 每一頁的四邊內距（W112：以 Space 頁為標準）。
    static let inset: CGFloat = 14
    /// 每一頁區塊之間的間距。
    static let sectionSpacing: CGFloat = 12
    /// 設定浮層外框；所有分頁同尺寸。
    static let shellWidth: CGFloat = 780
    static let shellHeight: CGFloat = 560
}

/// W112：設定每一頁共用的標題列。標題在左上、`.headline`；副標 `.caption` 次級色；
/// 右側只放那一頁自己的動作（例如「重新檢查」）。不自帶關閉鈕——點浮層外空白與 Esc 就是關閉。
struct TatwoSettingsPageHeader<Trailing: View>: View {
    let title: String
...
extension TatwoSettingsPageHeader where Trailing == EmptyView {
...
struct TatwoSettingsShell<Content: View>: View {
    ...
    var body: some View {
        ...
        // W112：十二頁共用同一個外框，換頁不會跳尺寸。
        .frame(width: TatwoSettingsPageMetrics.shellWidth,
               height: TatwoSettingsPageMetrics.shellHeight)
    }
    ...
    private var leftNav: some View {
            ...
            ForEach(Section.allCases) { item in
                Button {
                    ...
                    HStack(spacing: 9) {
                        ...
                        Spacer(minLength: 0)
                        if checklist.pendingSections.contains(item) {
                            Circle().fill(LiquidGlassTokens.brandAccent).frame(width: 6, height: 6)
                                .accessibilityLabel("有初始設定還沒做")
                        }
                    }
        ...
        .frame(width: BrowserSidebarMetrics.settingsNavWidth)   // W112：每一頁左欄同寬（原本瀏覽器頁 168、其餘 200）
...
struct TatwoSettingsPage: View {
    ...
    enum Section: String, CaseIterable, Identifiable {
        // W171：設定最上面一頁總覽要做的事（使用者：設定是否完善就看開始使用）。
        case start
        case space
        case issueList
    ...
    var body: some View {
        ...
        // 設定是視窗內的浮層、拿不到鍵盤焦點，離開指令的修飾器不會觸發；用鍵盤捷徑按鈕才收得到 Esc（自測 v2.0.12.006 實證）。
        .background {
            Button("關閉設定", action: onClose).keyboardShortcut(.cancelAction)
    ...
    private var issueListContent: some View {
        VStack(alignment: .leading, spacing: TatwoSettingsPageMetrics.sectionSpacing) {
            TatwoSettingsPageHeader(title: "Issue List")
