...
/// The retired repository remains only as a deny target for cached updater requests.
/// App updates never read account credentials or activate a private channel.
struct UpdateChannel {
    ...
    static func current() -> Self {
        Self(requestedPrivate: false, username: nil, token: nil)
    }
...
final class GitHubReleaseUpdateChecker: ObservableObject {
    ...
    var terminalInstallCommand: String {
        ...
        return "curl -fsSL https://raw.githubusercontent.com/\(repository)/\(tag)/install.sh | TATWO_OS_VERSION='\(tag)' bash"
    }
    ...
    var repository: String {
        // Feedback preferences and legacy markers must not redirect App updates.
