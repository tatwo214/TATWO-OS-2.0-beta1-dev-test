// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/DiffReviewView.swift；改動 23 行（原因：限定 DispatchCard 命名空間避開舊 stub；接同名解析型別、stat 滾動區、真實重讀、截斷與空 diff 提示）
import SwiftUI

extension DispatchCard {
...
struct DiffReviewView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                ...
                Text("子任務變更").font(.system(size: 13, weight: .black, design: .rounded))
                Spacer(minLength: 4)
                ...
                Button(action: { if let onReload { onReload() } else { reload() } }) {
            ...
            HStack(spacing: 6) {
                ...
                if let goalLabel { Text(goalLabel).font(.caption).foregroundStyle(.secondary) }
            }
            if let reviewerLabel {
                ScrollView(.vertical) {
                    Text(reviewerLabel).font(.system(size: 9, design: .monospaced))
    ...
    private var emptyState: some View {
        VStack(spacing: 8) {
            ...
            Text("沒有變更").font(.system(size: 12, weight: .semibold)).foregroundStyle(.secondary)
        }
