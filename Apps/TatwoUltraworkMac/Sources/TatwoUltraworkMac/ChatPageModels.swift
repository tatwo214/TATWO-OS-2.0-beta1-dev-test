...
enum ChatTurnContractEffortResolver {
    static func resolve(
        ...
        canonicalSourceSlotID: String? = nil,
        computerHostDecision: ChatComputerHostTurnDecision = .denied
        ...
        let matches = contract?.loopGovernorDecision.activatedBindings.filter { binding in
            binding.enabled
                ...
                && (canonicalSourceSlotID.map { binding.id == $0 }
                    ?? (binding.phase == phase))
                && binding.boundModelIDs.contains {
                    canonicalModelID($0) == canonicalRoute
                }
        } ?? []
        // Legacy contracts can issue identity slots without a governor topology.
        // Use only that exact issued slot; never bypass a populated governor.
        var fallbackSourceSlotID: String?
        if let canonicalSourceSlotID,
           let contract,
           contract.loopGovernorDecision.activatedBindings.isEmpty
        {
            let identities = contract.identityBindings.filter {
                $0.sourceSlotID == canonicalSourceSlotID
                    && $0.modelID.map(canonicalModelID) == canonicalRoute
            }
            if identities.count == 1 {
                fallbackSourceSlotID = canonicalSourceSlotID
            }
        }
        // Confirmed execution already owns an exact issued dispatch slot.
        // Its UI phase can advance to loops without changing that authority.
        if canonicalSourceSlotID != nil,
           matches.count != 1, fallbackSourceSlotID == nil
        {
            return ChatTurnDispatchSnapshot(
                routeID: route.id,
                canonicalModelID: canonicalRoute,
                ...
                blocker: "canonical dispatch 的 issued slot 無法唯一驗證；沒有派工。",
                computerHostDecision: computerHostDecision)
        }
        ...
        guard !route.allowedEfforts.isEmpty else {
            return ChatTurnDispatchSnapshot(
                ...
                phase: phase,
                contractID: contract?.contractID,
                contractBindingID: fallbackSourceSlotID,
                requestedEffort: nil,
                forwardedEffort: nil,
                effortOutcome: .noNativeEffortRequested,
                blocker: nil,
