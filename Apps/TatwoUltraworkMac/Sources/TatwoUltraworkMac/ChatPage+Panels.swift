...
extension ChatPage {
    ...
    func mainPane(contentMaxWidth: CGFloat?, forceCompactToolbar: Bool = false) -> some View {
        VStack(spacing: mainPaneVerticalSpacing) {
            if model.mode == .bot {
                // Gen-4 純 UI 展示面：fixture 場景經 export env 選擇（僅 snapshot
                // 模式生效），互動預設 rail-tree；零 runner、零落盤。
    ...
    func rightPanelControlStrip(showsThreadControls: Bool) -> some View {
        HStack(spacing: 8) {
            ...
            // 工具箱：Loops 工作串、變更收據、瀏覽器、檔案（收進 Menu）。
            let browserFileActive = (rightPanelContent == .browser || rightPanelContent == .file || rightPanelContent == .loops || rightPanelContent == .diff) && isRightPanelOpen
            Menu {
                Button { applyRightPanelInteraction(.toggleLoops) } label: {
                    Label("Loops 工作串", systemImage: "arrow.trianglehead.2.clockwise.rotate.90")
                }
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 7) {
                ...
                Text("\(model.visibleIssueListEntries.count)")
                    ...
                    .foregroundStyle(.tertiary)
                Button {
                    model.issueListShowsGlobal.toggle()
                } label: {
                    Label(model.issueListShowsGlobal ? "全域" : "本串",
                          systemImage: model.issueListShowsGlobal ? "globe" : "bubble.left")
                        .font(.system(size: 9, weight: .bold, design: .rounded))
                        .foregroundStyle(model.issueListShowsGlobal
                            ? LiquidGlassTokens.brandAccent
                            : .secondary)
                }
                .buttonStyle(.plain)
                .help(model.issueListShowsGlobal
                    ? "全域模式：顯示所有 thread 的 issue；點擊切回只看本串/本專案"
                    : "本串模式：只看此 thread 與同專案的 issue；點擊看全域")
                Spacer(minLength: 0)
                Button {
                ...
                .buttonStyle(.plain)
                .help("捕捉當前討論進 issue list（只入佇列，不啟動執行）")
                .accessibilityLabel("捕捉當前討論進 issue list")
            }
            // @ 搜尋點選的釘選項：跨 thread 單筆檢視、自動展開、✕ 解除。
            if let focused = model.issueListEntries.first(
                where: { $0.id == model.focusedIssueEntryID }) {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(focused.body)
                        ...
                        .fixedSize(horizontal: false, vertical: true)
                    Text("來源：\(focused.sourceType == .plan ? "Plan" : "Chat")・\(focused.sourceReference)")
                        .font(.system(size: 9))
                        .foregroundStyle(.tertiary)
                    HStack {
                        Spacer(minLength: 0)
                        issuePackIntoChatButton(focused)
                    }
                }
                .padding(7)
                .background(
                    LiquidGlassTokens.brandAccent.opacity(0.07),
                    in: RoundedRectangle(cornerRadius: 8, style: .continuous))
            }
            if model.visibleIssueListEntries.isEmpty {
                Text(model.issueListShowsGlobal
                    ? "全域佇列是空的；/issue <文字> 或按＋捕捉支線討論"
                    : "本串沒有 issue；@ 可搜全域佇列")
                    .font(.system(size: 11))
                    .foregroundStyle(.tertiary)
            }
                        ...
                        if let url = model.issueImageURL(relativeAssetPath: relativePath),
                           let image = NSImage(contentsOf: url) {
                            ZStack(alignment: .topTrailing) {
                                ...
                                Button {
                                    model.removeIssueImageNote(relativePath, from: entry)
                                } label: {
