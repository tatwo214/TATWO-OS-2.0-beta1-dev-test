...
struct DeviceSyncReceiptProgressView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(spacing: 7) {
                ...
                Text(presentation.percentLabel)
                    .font(.caption2.monospacedDigit().weight(.bold))
                    .foregroundStyle(tint)
                Text(presentation.progressBasisLabel)
                    .font(.caption2.weight(.semibold))
                    .foregroundStyle(
            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text(presentation.receipt.message)
                        .font(.caption2)
                        .foregroundStyle(.tertiary)
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    Text(presentation.attestationLabel)
                        .font(.caption2.monospaced())
                        .foregroundStyle(
                        ...
                        .textSelection(.enabled)
                    Text(presentation.receipt.attestationEvidence.detail)
                        .font(.caption2)
                        .foregroundStyle(.tertiary)
                        .fixedSize(horizontal: false, vertical: true)
                    if let artifactIssueLabel = presentation.artifactIssueLabel {
                        Text(artifactIssueLabel)
                            .font(.caption2.monospaced())
                            .foregroundStyle(.red)
                            .fixedSize(horizontal: false, vertical: true)
                            .textSelection(.enabled)
        ...
        HStack(alignment: .top, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                ProgressView(value: item.progressFraction)
                    ...
                    .tint(item.isVerified ? .green : .orange)
                ...
                ForEach(item.repositories) { repository in
                    HStack(alignment: .top, spacing: 6) {
                        ...
                        .font(.caption2)
                        .foregroundStyle(repository.isVerified ? .green : .orange)
    ...
    private func consumerReadbackRow(
        ...
        HStack(alignment: .top, spacing: 8) {
            ...
            .font(.caption)
            .foregroundStyle(consumer.isLoaded ? .green : .orange)
            .frame(width: 14)
    ...
    private var tint: Color {
        ...
        case .converged:
            ...
            return presentation.isConverged ? .green : .orange
        case .failed:
            return .red
        case .unreadable:
            return .red
