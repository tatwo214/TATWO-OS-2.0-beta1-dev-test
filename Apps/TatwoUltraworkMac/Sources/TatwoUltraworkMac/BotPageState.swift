...
// 全部 view-local fixture state；禁落盤、禁 runner/通道/sandbox。
...
final class BotPageState: ObservableObject {
    ...
    init(sceneID: String, contentMode: BotContentMode = .botThread, quickCardOpen: Bool = false) {
        ...
        self.selectedPrincipalID = self.fixture.selectedPrincipalID
        self.selectedSubID = self.fixture.selectedSubID
        self.selectedSpaceID = self.fixture.selectedSpaceID
        self.expandedPrincipalIDs = self.fixture.expandedPrincipalIDs
        self.quickCardOpen = quickCardOpen
        self.settingsSelection = self.fixture.selectedSubID ?? self.fixture.selectedPrincipalID
        self.pinnedBotIDs = self.fixture.pinnedBotIDs
        self.rowOrder = self.fixture.principals.map(\.id)
    }
    ...
    var currentThread: [BotFixtureMessage] {
        BotPageFixture.thread(principalID: selectedPrincipalID, subID: selectedSubID)
            ?? fixture.thread
    }
