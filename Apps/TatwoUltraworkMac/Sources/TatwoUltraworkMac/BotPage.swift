...
struct BotPageRootView: View {
    ...
    var body: some View {
        Group {
            if state.unknownScene {
                ...
                mainSlot
                    .padding(.leading, sidebarPinnedPref ? 250 : 0)
                    // 書側標籤耳（2026-08-22 使用者：「要在 app 外筐之外」）：
                    // 互動模式用 child window 掛在主窗右緣外側（視圖畫不出視窗外，
                    // 子視窗可以）；快照 export 抓不到子視窗 → export 模式退回窗內貼緣版。
                    ...
                    .overlay(alignment: .leading) {
                        if sidebarShown && !sidebarPinnedPref {
                            diaSidebar
                    ...
                    .overlay(alignment: .leading) {
                        if sidebarPinnedPref {
                            diaSidebar
                                .zIndex(40)
                        }
                    }
                    // 雙指滑動：資訊卡開著＝切卡頁（資訊卡⇄口袋，像 space switch）；
        ...
        .overlay {
            if state.contentMode == .settings { settingsOverlay }
        }
    ...
    private func scheduleSidebarHide() {
        ...
        let task = DispatchWorkItem { if !sidebarPinnedPref { sidebarShown = false } }
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 5) {
                ForEach(ChatRunMode.visibleChatTabs) { mode in
                    let isSelected = mode == .bot
                    // 2026-08-23 修：真實切換（原 no-op 害 bot 分頁切不回去）。
                    Button {
                    ...
                    .buttonStyle(.plain)
                    .help(mode == .bot ? "bot 展示" : "切到 \(mode.rawValue)")
                }
            }
            .padding(4)
            .chatGlassChip()
            ...
            // 底：space 圓點在「最左緣→TATWO OS 字符」這段空間置中（使用者紅圈），
            // 不做全寬置中，才不會跟字符擠。
            HStack(spacing: 0) {
                HStack(spacing: 8) {
    ...
    private var threadView: some View {
        ...
        VStack(spacing: 0) {
            // LINE 式：對話對象名稱放頂部正中央；右緣＝口袋開關（bot 的資訊卡）。
            ZStack {
                ...
                HStack {
                    ...
                    .buttonStyle(.plain)
                    .help("口袋：這隻 bot 的 plugins／skillet 登記")
                    .padding(.trailing, 14)
                }
            }
            .padding(.top, 12)
            .padding(.bottom, 8)
            if thread.isEmpty {
                BotEmptyHint(symbol: "bubble.left.and.text.bubble.right", text: "尚無對話")
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
            } else {
                ScrollView {
    ...
    // MARK: - 口袋（每隻 bot 的右側資訊卡；plugins／skillet 登記＋回傳）
    ...
    /// 第 1 頁＝bot 資訊卡（chat 同語言）、第 2 頁＝口袋；雙指左右滑或點底部圓點切頁。
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                ForEach(0..<2, id: \.self) { page in
                    ...
                    .buttonStyle(.plain)
                    .help(page == 0 ? "資訊卡（雙指左右滑切換）" : "口袋（雙指左右滑切換）")
                }
            }
            .padding(.vertical, 8)
        }
    ...
    private var botInfoPage: some View {
        VStack(alignment: .leading, spacing: 8) {
            botSummarySection
            botOutputFilesCard(files: ["社群貼文/週五背部滿版.md", "素材庫/過程照-01.jpg"], total: 2)
            ...
            botInfoContextStrip(
                ...
                tint: .secondary,
                items: [
                    ("Project", state.selectedPrincipal?.name ?? "—", "folder"),
                    ("Branch", state.selectedSpace?.name ?? "—", "arrow.branch"),
                    ("Thread", String((state.pocketBotKey ?? "—").suffix(8)), "number"),
                ])
    ...
    private var botSummarySection: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 6) {
                botInfoChip(systemImage: "person.2", text: "bot · 未接入")
    ...
    private var botPluginSummaryStrip: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            .buttonStyle(.plain)
            .help("skillet 登記＝授權引用主根；口袋在第 2 頁（雙指右滑）。")
    ...
    private var botIssueListSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            ForEach(["背部滿版貼文待過目（展示）", "閃預約折扣標籤確認（展示）"], id: \.self) { title in
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 6) {
                        Circle()
    ...
    /// 第 2 頁：口袋（plugins／skillet 登記＋回傳）。
    private var pocketContent: some View {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 1) {
                        Text("口袋").font(.system(size: 13, weight: .bold))
    ...
    private var botComposer: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            // 「/skill」：列出口袋已登記的 skills（chat 斜線列同語言）。
    ...
    /// /skill 供應列：口袋已登記 skills 的 chip（點擊帶入輸入框；展示）。
    private var botSkillRail: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ...
                if state.registeredSkills.isEmpty {
                    Text("口袋還沒登記 skills——開右上「口袋」搜尋 skillet 登記")
...
fileprivate struct BotSideTabTrapezoid: Shape {
...
@MainActor
fileprivate enum BotSideTabTheme {
...
@MainActor
fileprivate final class BotEdgeTabsPanelController {
    ...
    func attach(to window: NSWindow, state: BotPageState) {
        ...
        panel.contentView = NSHostingView(rootView: BotEdgeTabsPanelView(state: state))
...
fileprivate final class BotEdgeTabsTrackerView: NSView {
