...
private struct TatwoHydratedPanelView: View {
    ...
    var body: some View {
        ...
        .onChange(of: selection) { _, newPage in
            ...
            retainedChatLifecycle.pageSelectionChanged(isChatSelected: newPage == .chat)
            if newPage == .chat {
        ...
        .confirmationDialog(
            ...
            if let proposal = authorityBootstrapModel.pendingProposal {
                Text(
                    "Disposition: \(proposal.bootstrapDispositionPreview)\n"
    ...
    private func pageContent(for page: TatwoPage) -> some View {
        ...
        case .workflow:
            WorkflowPage(snapshot: snapshot)
        }
    }
}
