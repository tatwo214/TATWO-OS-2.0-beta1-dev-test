...
struct WorkOSModeRouteStrip: View {
    let selectedMode: WorkModeID
    @Environment(\.tatwoSurfaceKind) private var surface

    private var routes: [TatwoOSModeRouteV1] {
        surface == .panel
            ? TatwoOSModeRouteCatalog.all.filter { $0.mode == selectedMode }
            : TatwoOSModeRouteCatalog.all
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(spacing: 8) {
                Label("Codex App 新增 OS 模式", systemImage: "switch.2")
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: surface == .panel ? 170 : 210), spacing: 7)], spacing: 7) {
                ForEach(routes) { route in
                    WorkOSModeRouteTile(route: route, selected: route.mode == selectedMode)
                }
            }
        }
        .padding(10)
...
struct WorkOSModeRouteTile: View {
    @ObservedObject private var themeStore = TatwoThemeStore.shared
    let route: TatwoOSModeRouteV1
    let selected: Bool

    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(spacing: 6) {
                Text(route.mode.rawValue)
                    .font(.system(size: 11, weight: .black, design: .rounded))
                    .foregroundStyle(.white)
                    .frame(width: 28, height: 22)
                    .background(routeColor, in: RoundedRectangle(cornerRadius: 8, style: .continuous))
            ...
            Text(route.plainPurpose)
                .font(.system(size: 9.2, weight: .semibold, design: .rounded))
                .foregroundStyle(.secondary)
                .lineLimit(2)
            HStack(spacing: 6) {
                Badge(route.speedDefault)
                Badge(route.status.rawValue.replacingOccurrences(of: "_", with: " "))
                Spacer(minLength: 0)
            }
        }
        .padding(9)
        .frame(maxWidth: .infinity, minHeight: 82, alignment: .topLeading)
        .background(routeColor.opacity(selected ? 0.15 : 0.065), in: RoundedRectangle(cornerRadius: 13, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: 13, style: .continuous)
                .strokeBorder(routeColor.opacity(selected ? 0.50 : 0.16), lineWidth: selected ? 1.4 : 1)
        )
    }
