...
test('browser rejoins the shared chat sidebar shell and footer with no private shell', () => {
  ...
  assert.doesNotMatch(controls, /sidebarPinnedPref\.toggle\(\)/);
...
test('v6 sidebar has five ordered sections, white selection and no chat or search pane', () => {
  ...
  assert.match(design, /Text\("Pinned"\)\.fontWeight\(\.bold\)/);   // 使用者 09-20：釘選分頁改「Pinned」
...
@main struct Fixture {
    @MainActor static func main() {
        ...
        // W115：拖進資料夾＝分頁搬進書籤列（不留在下面的清單）。
        precondition(store.folders[0].bookmarks.count == 2 && store.activeTabs.isEmpty)
        precondition(store.tab(forBookmark: store.folders[0].bookmarks[1].id)?.id == tabID)
        // 後面的情境需要一個一般分頁：把它移出書籤（分頁留著），再補回一個同網址的書籤維持數量。
        let draggedURL = store.folders[0].bookmarks[1].url
        registry.removeBookmark(store.folders[0].bookmarks[1].id)
        precondition(registry.addBookmark(folderID: folderID, url: URL(string: draggedURL)!, title: "Fixture") != nil)
        precondition(store.tabs.map(\\.id) == originalTabs.map(\\.id) && store.activeTabs.count == 1 && store.folders[0].bookmarks.count == 2)
        ...
        precondition(store.tabs.map(\\.id) == originalTabs.map(\\.id) && store.folders.count == 3)   // W115：拖進又移出後 folderID 留著，其餘不變
...
@MainActor final class BrowserWorkSpaceRuntime: ObservableObject {
    ...
    func translate(tabID: UUID, operation: String, payload: String? = nil, limit: Int = 0) async -> String? { nil }   // W112
    var foregroundTabRequest: (tabID: UUID?, serial: Int) = (nil, 0)   // W114
...
`);
  ...
  const viewSources = ['-num-threads', '2', writeBrowserVisualTokens(dir, { includeOmnibox: true }), ...registrySources, stubs, toolbar,
    ...
    ...['BrowserAudibleTabs.swift', 'BrowserSpaceMenu.swift', 'BrowserChromeReveal.swift', 'BrowserPageTranslation.swift', 'BrowserSidebarDrops.swift', 'BrowserChromeStyleEmbed.swift', 'BrowserExtensionInventory.swift'].map(name => join(root, 'App/Sources/Tatwo2/Browser', name)),
