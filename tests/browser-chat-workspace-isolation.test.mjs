...
test('PR4c 聊天旁頂列固定在網頁上方（不浮在 CEF 上），chrome 一律擁有點擊', () => {
  ...
  assert.match(design, /if onClose != nil \{[\s\S]*?VStack\(spacing: BrowserOmniboxMetrics.zero\) \{[\s\S]{0,400}?workspaceToolbar[\s\S]*?browserPageContent\s*\}/);   // 中間有註解；以前是靠獨立 Browser 那個 VStack 才比對得到
