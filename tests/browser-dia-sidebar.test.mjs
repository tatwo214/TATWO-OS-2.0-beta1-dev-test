...
test('W66 bookmark rows project the bound tab and stay out of both ordinary tab lists', () => {
  // W112：珍藏的分頁也一樣住在自己的格子上，不列在下面。
  assert.match(design, /var activeTabs: \[Tab\] \{ tabs.filter \{ !\$0.pinned && \$0.bookmarkID == nil && \$0.favoriteID == nil \} \}/);
  assert.match(design, /var pinnedTabs: \[Tab\] \{ tabs.filter \{ \$0.pinned && \$0.bookmarkID == nil && \$0.favoriteID == nil \} \}/);
...
test('W66 hover minus closes tabs only and remains keyboard/accessibility reachable', () => {
  ...
  // W112：書籤的右鍵選單（含「刪除書籤」）搬進這個檔；「hover 的減號只關分頁」要驗的是那顆鈕本身。
  assert.match(rows, /label: "關閉這個書籤的分頁", identifier: "browser\.bookmark\.close\.\\\(bookmark\.id\)"\) \{\s*store\.closeBookmark\(bookmark\.id\)/);
  assert.doesNotMatch(rows.slice(rows.indexOf('private struct BrowserBookmarkCloseButton')), /deleteBookmark|removeBookmark|removeSpace/);
  assert.match(rows, /contextMenu \{[\s\S]*Button\("刪除書籤"\) \{\s*store.deleteBookmark/);   // W112：選單搬進 BrowserBookmarkRow
});
...
test('W66 space capsule overlays the outer shell, not the padded mode section, with one height frame', () => {
  ...
  // W114：不再用算出來的常數；直接量紅綠燈關閉鈕的中心線（使用者：「喬很多次」）。
  assert.match(shell, /TrafficLightAlignedTitle\(height: WorkspaceSidebarMetrics\.spaceSwitcherHeight\)/);
  ...
  // W112：Chat 的側欄固定鈕坐在側欄右緣外側，拖曳區多讓出那一格。
  assert.match(rail, /frame\(width: showRightPanelToggle \? WindowChromeMetrics.trafficLightSafeWidth : WorkspaceSidebarMetrics.width \+ ChatPage.sidebarPinButtonReserve\)\s*\.allowsHitTesting\(false\)/);
});

test('W66 live Bot and Browser share the approved dot-plus ratio and centered scroll group', () => {
  // W112：空間圓點搬到 BrowserSpaceMenu.swift，圓點尺寸 token 跟著過去；這裡把兩段合起來驗同一套比例。
  const browser = section(design, 'private var spaceControls:', 'private var downloadsPopover:') + read(app + 'Browser/BrowserSpaceMenu.swift').split('private struct BrowserSpaceMenu')[0];
