...
test('W97 (d) our --disable-features entries are unchanged; CEF\'s own protection list is restated after them', () => {
  // Baseline entries: beta1/integration @ 7a2c6b3d. W97 may not widen, narrow or reorder them.
  // W116c/d (its own train): kDeniedHostSwitches removes disable-features (untrusted argv), which also wiped the
  // list CEF adds before calling us (GlicActorUi crash, issue #3982). We restate those four names ourselves.
  assert.equal(bridge.split('"AutofillServerCommunication,MediaRouter,WebBluetooth,WebHID,"').length - 1, 1);
  assert.equal(bridge.split('"WebNFC,WebOTP,WebSerial,WebUSB,"').length - 1, 1);
  assert.equal(bridge.split('"GlicActorUi,AutofillActorMode,LensOverlay,KillOnInvalidNavigationHeaders,"').length - 1, 1);
  // W150：Gemini in Chrome（Glic）整個關掉——它的快捷鍵管理器在 profile 關閉時踩到沒有工具列的擴充視窗而當機。
  assert.match(bridge, /KillOnInvalidNavigationHeaders,"\n(?:\s*\/\/[^\n]*\n)*\s*"Glic";/);
  assert.match(bridge, /command_line->AppendSwitchWithValue\("disable-features", disabled_features\);/);
  assert.doesNotMatch(bridge, /GetSwitchValue\("disable-features"\)/, 'never trust an inherited value');
