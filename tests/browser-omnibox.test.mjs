...
test('W67 Dia toolbar reserves layout height so native page cannot overlap controls', () => {
  ...
  // W112：獨立 Browser 的頂列是浮層（平時隱形）；聊天旁仍是 VStack 佔高度。
  assert.match(workspace, /ZStack\(alignment: \.top\) \{\s*browserPageContent[^\n]*\s*if chromeReveal\.revealed/);
...
test('W67 collapse shows committed host only and never presents HTTP as locked', () => {
  ...
  assert.match(trigger, /Text\(labelOverride \?\? BrowserOmniboxPresentation.domain\(for: state.urlString\)\)/);
...
test('/goal 101: chat-side address editing is inline in the toolbar row, not a floating panel over the page', () => {
  ...
  assert.match(toolbar, /\.onChange\(of: isExpanded\) \{ _, expanded in isEditing\.wrappedValue = expanded \}/);   // W115：獨立 Browser 也回報（隱形頂列要靠它留住面板）
