...
if cmd == "state": print(WX, WY, W, H, state())
    ...
    # W112：合成滑鼠移動（不動真的游標）；座標是視窗內、左上為原點。
    r = probe("move", WX + float(argv[2]), WY + float(argv[3])); time.sleep(0.6)
    print("move:", r.get("posted"), (r.get("chain") or ["?"])[0].split(" canMove")[0])
elif cmd == "hit":
    print((probe("hit", WX + float(argv[2]), WY + float(argv[3])).get("chain") or ["?"])[:3])
elif cmd in ("hmove", "hclick"):
    # W115：系統層（HID）事件——真的移動游標／真的點下去；只在使用者離開時用。座標＝視窗內、左上原點。
    r = rpc("ui_probe", {"action": "hid_move" if cmd == "hmove" else "hid_click", "x": WX + float(argv[2]), "y": WY + float(argv[3])}, timeout=15)
    time.sleep(1.2); print(cmd, r.get("result") or r, state())
elif cmd == "hdrag":
    r = rpc("ui_probe", {"action": "hid_drag", "x": WX + float(argv[2]), "y": WY + float(argv[3]), "x2": WX + float(argv[4]), "y2": WY + float(argv[5])}, timeout=15)
    time.sleep(2.5); print("hdrag", r.get("result") or r, state())
